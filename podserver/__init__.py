...
def create_app(config_file='config.yml'):
    ...
    try:
        for entry in config.app_config['networks']:
            network = Network(entry)
            config.networks[network.id] = network

    except KeyError as exc:
        _LOGGER.exception(
            'Each network must have a name and a number: %s', exc
        )
    # config.server = Server(
    #    config.app_config['server'],
    #    config.app_config['application']['cache_server'],
    #    config.networks
    # )

    if not os.environ.get('SERVER_NAME') and config.server.name:
        os.environ['SERVER_NAME'] = config.server.name
