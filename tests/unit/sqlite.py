...
class TestAccountManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_object(self) -> None:
        ...
        sql: SqliteStorage = data_store.backend

        # Populate Person object with string data and check the result
        person_table: SqlTable = sql.member_sql_tables[uuid]['person']
        given_name = 'Steven'
        family_name = 'Hessing'
        ...
        # result
        member_table: SqlTable = sql.member_sql_tables[uuid]['member']
    ...
    async def test_array(self) -> None:
        ...
        account: Account = server.account
        service_id: int = ADDRESSBOOK_SERVICE_ID
        member: Member | None = await account.get_membership(service_id)
        uuid: UUID = member.member_id
        ...
        network_invites_table: SqlTable = \
            sql.member_sql_tables[uuid]['network_invites']
        ...
        for row in data:
            ...
            for column_name in row.keys():
                if not column_name.startswith('_'):
                    continue
                field_name: str = column_name.lstrip('_')
                result[field_name] = \
        ...
        data_filter = DataFilterSet(
            {
                'created_timestamp': {
                    'after': datetime.now(UTC) - timedelta(days=3650)
                }
            }
        )
        data = await network_invites_table.delete(
