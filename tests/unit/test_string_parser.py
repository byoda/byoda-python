...
class TestSplitQuotedString(unittest.TestCase):
    ...
    def test_basic_string(self) -> None:
        ...
        result: list[str] = split_quoted_string('onetwothree')
        expected: list[str] = ['onetwothree']
        self.assertEqual(result, expected)

    def test_basic_comma_separated_with_quotes(self) -> None:
        '''Test basic comma-separated values with quotes'''
        result: list[str] = split_quoted_string('foo, bar, "hello world", baz')
        expected: list[str] = ['foo', 'bar', 'hello world', 'baz']
        self.assertEqual(result, expected)

    def test_space_separated_with_double_quotes(self) -> None:
        '''Test space-separated with double quotes'''
        result: list[str] = split_quoted_string('"test one" test2 "test three"')
        expected: list[str] = ['test one', 'test2', 'test three']
        self.assertEqual(result, expected)

    def test_single_quotes(self) -> None:
        '''Test single quotes'''
        result: list[str] = split_quoted_string(
            "'single' and 'double quotes' work"
        )
        expected: list[str] = ['single', 'and', 'double quotes', 'work']
        self.assertEqual(result, expected)
    ...
    def test_mixed_quotes(self) -> None:
        ...
        expected: list[str] = ['foo', 'bar baz', 'qux quux']
        self.assertEqual(result, expected)
    ...
    def test_no_quotes(self) -> None:
        ...
        result: list[str] = split_quoted_string('one, two, three')
        expected: list[str] = ['one', 'two', 'three']
        self.assertEqual(result, expected)
    ...
    def test_quotes_at_beginning_and_end(self) -> None:
        ...
        result: list[str] = split_quoted_string('"first", "second", "third"')
    ...
    def test_custom_delimiter(self) -> None:
        ...
        expected: list[str] = ['one', 'two', 'three four', 'five']
        self.assertEqual(result, expected)
    ...
    def test_complex_youtube_keywords(self) -> None:
        ...
        result: list[str] = split_quoted_string(keywords_str)
        expected: list[str] = [
            'legal analysis', 'big law', 'lsat', 'personal injury lawyer',
            'supreme court', 'law firm', 'law school', 'law and order',
            'lawyers', 'legal eagle', 'lawyer reacts', 'ace attorney',
    ...
    def test_nested_quotes_different_types(self) -> None:
        ...
        expected: list[str] = ['one', "two 'nested' three", 'four']
        self.assertEqual(result, expected)
