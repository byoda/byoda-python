...
sudo mkdir -p ${BYODA_ROOT_DIR}/${POSTFIX} /var/lib/postgresql/${POSTFIX}/data
if [ -z "${LOGLEVEL}" ]; then
...
export ROOT_DIR=${BYODA_ROOT_DIR}

if [ -z "${HOST_ROOT_DIR}" ]; then
    export HOST_ROOT_DIR=${BYODA_ROOT_DIR}
fi
...
ACCOUNT_SECRET_PRESENT=$(grep -c ACCOUNT_SECRET ${ENV_FILE})
if [ "${ACCOUNT_SECRET_PRESENT}" == "0" ]; then
